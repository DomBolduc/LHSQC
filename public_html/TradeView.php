...
if (empty($TradeFromTeam) == false){while ($Row = $TradeFromTeam ->fetchArray()) {	
	...
	// Vérifier l'approbation dans la table séparée
	$TradeApprovalDBFile = "LHSQC-TradeApproval.db";
	if(file_exists($TradeApprovalDBFile)){
		$approvalDB = new SQLite3($TradeApprovalDBFile);
		$checkApproval = $approvalDB->querySingle("SELECT CommissionerApproved FROM TradeCommissionerApproval WHERE FromTeam = " . $Team . " AND ToTeam = " . $ToTeam, true);
		$approvalDB->close();
	}else{
		$checkApproval = null;
	}
	
	if($checkApproval && $checkApproval['CommissionerApproved'] == 'True'){
	...
	If ($TradeMain != null && $AlreadyShow[$Team][$TradeMain['ToTeam']] == ""){
