...
<!-- Inclusion du CSS moderne pour PlayersRoster -->

<link rel="stylesheet" href="css/players-roster-modern.css">
...
If (file_exists($DatabaseFile) == false){	Goto STHSErrorPlayerRoster;
	...
	If($Year > 0 AND file_exists($CareerStatDatabaseFile) == true){  /* CareerStat */
		...
		If ($CareerDBFormatV2CheckCheck['CountName'] == 1){
			...
			if ($FreeAgentYear == -1) {
				$Query = $Query . " AND PlayerInfoHistory.Contract > 0";
			}
		...
		if ($FreeAgentYear == -1){
			$Query = $Query . " AND PlayerInfo.Contract > 0";
		}
...
$(function() {
  ...
  $(".STHSPHPAllPlayerRoster_Table").tablesorter({
    ...
    widgetOptions : {
      ...
      columnSelector_mediaquery: false, // Désactiver le masquage automatique sur mobile
      ...
      columnSelector_mediaqueryState: false, // Désactiver l'état automatique



      columnSelector_mediaqueryHidden: false, // Ne pas masquer automatiquement
...
<div class="players-roster-container">

    <div class="players-roster-header">
    ...
    <div class="alphabet-selector">
        ...
        <h3>Filtrer par première lettre du nom</h3>

        <div class="alphabet-buttons">

            <button class="alphabet-btn all" data-letter="all">TOUS</button>

            <?php

            // Générer les boutons A-Z
            ...
            for ($i = ord('A'); $i <= ord('Z'); $i++) {
                ...
                echo "<button class=\"alphabet-btn{$activeClass}\" data-letter=\"{$letter}\">{$letter}</button>";
    ...
    <div class="filter-stats" id="filter-stats">
        ...
        <span id="visible-count">0</span> joueur(s) affiché(s) sur <span id="total-count">0</span> au total
    ...
    <div class="roster-table-container">
        <div class="table-scroll-wrapper">
            <table class="table tablesorter STHSPHPAllPlayerRoster_Table"><thead><tr>
...
if (empty($PlayerRoster) == false){while ($Row = $PlayerRoster ->fetchArray()) {
	...
	// Extraire la première lettre du nom pour le filtrage alphabétique

	$firstLetter = strtoupper(substr(trim($Row['Name']), 0, 1));



	echo "<tr class=\"player-row\" data-player-name=\"" . htmlspecialchars($Row['Name']) . "\" data-first-letter=\"{$firstLetter}\"><td><a href=\"PlayerReport.php?Player=" . $Row['Number'] . "\">" . $strTemp . "</a></td>";
...
?>
        ...
        </div> <!-- Fermeture table-scroll-wrapper -->
    </div> <!-- Fermeture roster-table-container -->

</div> <!-- Fermeture players-roster-container -->
...
// JavaScript pour la sélection alphabétique
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    totalCountElement.textContent = playerRows.length;
    ...
    function updateStats() {
        ...
        const visibleRows = document.querySelectorAll('.player-row:not(.hidden)');
    ...
    function filterPlayers(letter) {
        ...
        playerRows.forEach(row => {
            ...
            if (letter === 'all') {
                ...
                const playerLetter = row.getAttribute('data-first-letter');
    ...
    // Initialiser l'affichage avec la lettre "A" sélectionnée
...
// Gestion de l'indicateur de scroll horizontal sur mobile
document.addEventListener('DOMContentLoaded', function() {
    const scrollWrapper = document.querySelector('.table-scroll-wrapper');

    if (scrollWrapper && window.innerWidth <= 768) {
        // Fonction pour vérifier si on est au bout du scroll
        ...
        // Vérifier au scroll
        scrollWrapper.addEventListener('scroll', checkScrollEnd);
