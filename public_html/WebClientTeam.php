...
If ($Team == 0 OR $Team > 100){
		...
		If ($EditType == 1){
			...
			try {
				...
				If ($PlayerNumber > 0 and $PlayerNumber <= 10000){
					...
					$Query = "Update PlayerInfo SET AvailableForTrade = '" . $PlayerAvailableForTrade . "', WebClientModify = 'True' WHERE Number = " . $PlayerNumber;
					...
					$Query = "Update GoalerInfo SET AvailableForTrade = '" . $PlayerAvailableForTrade . "', WebClientModify = 'True' WHERE Number = " . ($PlayerNumber - 10000);
	...
	$Query = "SELECT MainTable.* FROM (SELECT PlayerInfo.Number, PlayerInfo.Name, PlayerInfo.Team, PlayerInfo.AvailableForTrade, PlayerInfo.PosC, PlayerInfo.PosLW, PlayerInfo.PosRW, PlayerInfo.PosD, 'False' AS PosG, PlayerInfo.Retire as Retire FROM PlayerInfo WHERE Team = " . $Team . " AND Retire = \"False\" UNION ALL SELECT GoalerInfo.Number, GoalerInfo.Name, GoalerInfo.Team, GoalerInfo.AvailableForTrade, 'False' AS PosC, 'False' AS PosLW, 'False' AS PosRW, 'False' AS PosD, 'True' AS PosG, GoalerInfo.Retire as Retire FROM GoalerInfo WHERE Team = " . $Team . " AND Retire = \"False\") AS MainTable ORDER BY MainTable.Name ASC";
