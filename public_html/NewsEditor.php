	...
	if ($NewsID >= 0 && isset($_POST["Erase"]) && $CookieTeamNumber > 0) {
		...
		If ($NewsID >= 0){
			...
			If ($NewsOwner != Null){
				...
				If ($HashMatch == True){
					/* Update Existing NewsID using prepared statement */
					$sql = "UPDATE LeagueNews SET Title = ?, Message = ?, WebClientModify = 'True' WHERE Number = ?";
					$stmt = $dbNews->prepare($sql);

					if ($stmt) {
						$cleanTitle = filter_var($_POST["Title"], FILTER_UNSAFE_RAW, FILTER_FLAG_STRIP_LOW || FILTER_FLAG_STRIP_HIGH || FILTER_FLAG_NO_ENCODE_QUOTES || FILTER_FLAG_STRIP_BACKTICK);
						$cleanMessage = $_POST["editor1"];

						$stmt->bindValue(1, $cleanTitle, SQLITE3_TEXT);
						$stmt->bindValue(2, $cleanMessage, SQLITE3_TEXT);
						$stmt->bindValue(3, $NewsID, SQLITE3_INTEGER);
						...
						if ($result) {
							...
							$InformationMessage = "Erreur lors de la mise à jour : " . $dbNews->lastErrorMsg();
						}

						$stmt->close();
					} else {
						$InformationMessage = "Erreur de préparation de la requête : " . $dbNews->lastErrorMsg();
					}
				}else{
					/* Hash do not Match */
					$InformationMessage = $NewsLang['IllegalAction'];
			...
			If ($HashMatch == True){
				...
				$Query = "INSERT INTO LeagueNews (Time,TeamNumber,TeamNewsNumber,Owner,Title,Message,Remove,WebClientModify,AnswerNumber) VALUES(?, ?, '0', ?, ?, ?, 'False', 'True', ?)";
				$stmt = $dbNews->prepare($Query);

				if ($stmt) {
					$currentTime = gmdate('Y-m-d H:i:s');
					$cleanOwner = filter_var($Owner, FILTER_UNSAFE_RAW, FILTER_FLAG_STRIP_LOW || FILTER_FLAG_STRIP_HIGH || FILTER_FLAG_NO_ENCODE_QUOTES || FILTER_FLAG_STRIP_BACKTICK);
					$cleanTitle = filter_var($_POST["Title"], FILTER_UNSAFE_RAW, FILTER_FLAG_STRIP_LOW || FILTER_FLAG_STRIP_HIGH || FILTER_FLAG_NO_ENCODE_QUOTES || FILTER_FLAG_STRIP_BACKTICK);
					$cleanMessage = $_POST["editor1"];

					$stmt->bindValue(1, $currentTime, SQLITE3_TEXT);
					$stmt->bindValue(2, $NewsTeam, SQLITE3_INTEGER);
					$stmt->bindValue(3, $cleanOwner, SQLITE3_TEXT);
					$stmt->bindValue(4, $cleanTitle, SQLITE3_TEXT);
					$stmt->bindValue(5, $cleanMessage, SQLITE3_TEXT);
					$stmt->bindValue(6, $ReplyNews, SQLITE3_INTEGER);
					...
					if ($result) {
						...
						$InformationMessage = "Erreur lors de la sauvegarde : " . $dbNews->lastErrorMsg();
					}
