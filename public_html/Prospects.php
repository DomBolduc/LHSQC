...
<!-- Inclusion du CSS moderne pour Prospects -->
<link rel="stylesheet" href="css/prospects-modern.css">

<script src="STHSMain.js" type="text/javascript"></script>

<?php If ($lang == "fr"){include 'LanguageFR-League.php';}else{include 'LanguageEN-League.php';}
...
<div class="alphabet-selector">
    <h3><?php echo ($lang == "fr") ? "Filtrer par première lettre du nom" : "Filter by first letter"; ?></h3>
    <div class="alphabet-buttons">
        <button class="alphabet-btn active" data-letter="all"><?php echo ($lang == "fr") ? "TOUS" : "ALL"; ?></button>
        <?php
        for ($i = ord('A'); $i <= ord('Z'); $i++) {
            $letter = chr($i);
            echo "<button class=\"alphabet-btn\" data-letter=\"{$letter}\">{$letter}</button>";
        }
        ?>
    </div>
</div>

<div class="filter-stats" id="prospects-filter-stats">
    <span id="prospects-visible-count">0</span>
    <?php echo ($lang == "fr") ? "prospect(s) affiché(s) sur" : "prospect(s) shown out of"; ?>
    <span id="prospects-total-count">0</span>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    const alphabetButtons = document.querySelectorAll('.alphabet-selector .alphabet-btn');
    const prospectRows = document.querySelectorAll('.prospect-row');
    const visibleCountElement = document.getElementById('prospects-visible-count');
    const totalCountElement = document.getElementById('prospects-total-count');

    if (!alphabetButtons.length || !prospectRows.length || !visibleCountElement || !totalCountElement) {
        return;
    }

    totalCountElement.textContent = prospectRows.length.toString();

    function updateStats() {
        const visibleRows = Array.from(prospectRows).filter(row => !row.classList.contains('hidden'));
        visibleCountElement.textContent = visibleRows.length.toString();
    }

    function filterProspects(letter) {
        const normalizedLetter = letter === 'all' ? 'all' : letter.toUpperCase();
        ...
        prospectRows.forEach(row => {
            ...
            const rowLetter = (row.getAttribute('data-first-letter') || '').toUpperCase();
            if (rowLetter === normalizedLetter) {
                row.classList.remove('hidden');
            } else {
    ...
    alphabetButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            filterProspects(this.dataset.letter || 'all');
        });
    });
