...
<form id="Trade" name="Trade" method="post" action="TradeOtherTeam.php<?php If ($lang == "fr" ){echo "?Lang=fr";}?>">
	...
	if (empty($Trade) == false){while ($Row = $Trade ->fetchArray()) {
		...
		If ($Row['Player']> 0 and $Row['Player']< 10000){
			...
			// NE PAS transférer les joueurs ici - cela sera fait par le simulateur seulement après l'approbation du commissaire
			...
			// NE PAS transférer les gardiens ici - cela sera fait par le simulateur seulement après l'approbation du commissaire
	...
	If ($Confirm == True){
		...
		// NE PAS mettre ConfirmTo = 'True' ici - cela sera fait quand le commissaire approuvera
		// Stocker la confirmation de Team2 dans la base séparée
		$TradeApprovalDBFile = "LHSQC-TradeApproval.db";
		$approvalDB = new SQLite3($TradeApprovalDBFile);
		$createTable = "CREATE TABLE IF NOT EXISTS TradeCommissionerApproval (ApprovalID INTEGER PRIMARY KEY AUTOINCREMENT, FromTeam INTEGER NOT NULL, ToTeam INTEGER NOT NULL, Team2Confirmed TEXT DEFAULT NULL, CommissionerApproved TEXT DEFAULT NULL, ApprovalDate DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(FromTeam, ToTeam))";
		$approvalDB->exec($createTable);
		// FromTeam = l'équipe qui a initié le trade, ToTeam = l'équipe qui confirme (Team2)
		$Query = "INSERT OR REPLACE INTO TradeCommissionerApproval (FromTeam, ToTeam, Team2Confirmed) VALUES (" . $TradeMain['FromTeam'] . ", " . $TradeMain['ToTeam'] . ", 'True')";
		try {
			$approvalDB->exec($Query);
			$approvalDB->close();
		} catch (Exception $e) {
			$InformationMessage = "Erreur: " . $e->getMessage();
		}
		// $Query = "UPDATE TRADE SET ConfirmTo = 'True' WHERE ToTeam = " . $Team . " AND FromTeam = " . $TradeMain['ToTeam'];
		// Code retiré - ConfirmTo sera mis à 'True' seulement quand le commissaire approuvera
